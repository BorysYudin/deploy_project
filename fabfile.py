    ...
    if True or host.sudo('service docker status', hide='out',
        ...
        with host.cd('awsdemo'):
            ...
            while True:
                ...
                time.sleep(2)
            host.run(
                '''docker exec $(docker ps -q -f name=awsdemo_web) /usr/src/app/manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser(username='%s', email='%s', password='%s')"''' % (
                DEFAULT_USER, DEFAULT_LOGIN, DEFAULT_PASS))
    else:
...
def pgdump(c):
    ...
    host.run(
        '''docker container exec %s sh -c "pg_dump -U awsdemo awsdemo | gzip > '/var/lib/postgresql/backups/awsdemo.gz'"''' % cid)
    host.run(
        'docker cp %s:/var/lib/postgresql/backups/awsdemo.gz /tmp/awsdemo.gz' % cid)
    t = Transfer(host)
